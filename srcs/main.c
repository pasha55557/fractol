...
/*   Updated: 2019/10/08 20:55:30 by rsticks          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Burningship, Mandelbar, tanhjulia");
		exit(1);
	}
}
...
{
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win,
	30, 30, 0xFFFFFF, "press +, mouse wheel to zoom");
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win,
	30, 50, 0xFFFFFF, "press -, mouse wheel to dezoom");
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win,
	30, 70, 0xFFFFFF, "press R to reboot fractal");
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win, 30,
	90, 0xFFFFFF, "press arrows to move fractal");
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win, 30,
	110, 0xFFFFFF, "press PAGE UP to change fractal");
	mlx_string_put(fractol->mlx.mlx, fractol->mlx.win, 30,
	130, 0xFFFFFF, "press NUM 1,2,3,4,5,6 to change color");
	if (fractol->id == 2 || fractol->id == 5)
		mlx_string_put(fractol->mlx.mlx, fractol->mlx.win, 30,
		1000, 0xFFFFFF, "press SPACE to magic");
}
...
static void		init_mlx(t_fractol *fractol)
{
	...
	fractol->mlx.img = mlx_new_image(fractol->mlx.mlx,
	WIDTH, HEIGHT);
	fractol->mlx.data = (int*)mlx_get_data_addr(fractol->mlx.img,
	&fractol->mlx.bpp, &fractol->mlx.line_size, &fractol->mlx.endiane);
	fractol->mlx.bpp /= 8;
}
